...
%    size and a given symmetry type
% 
% gridsize: grid size. For ud/lr/udlr/centro, gridsize=[height, width] 
%    For other symmetry types, it is required that height=width. If this 
%    input is a scalar N, it is regarded as a square grid, with 
%    N=height=width. In the current version, both width and height have 
%    to be even.
% 
% H: orthogonal matrix H (scaled version of the butterfly)
%
% H1: butterfly matrix H, with elements 0,1,-1
%
% symtype: grid symmetry type: 'ud', 'lr', 'udlr', 'centro', 'ds', 
%    'as', 'bs', 'penta'
% 
...
switch symtype
    ...
    case {'bs', 'penta'}
        ...
        idx_db=[idx_diags(:)', idx_l, idx_d, idx_r, idx_u];
        P_db = I_NN(:,idx_db);
...
switch symtype
    ...
    case 'ds'
        idx_d1 = zeros(1,NN);
        idx_d1(1:h)=diag(matI,0)';
        curI=h;
        for i=[-h+1:-1, 1:h-1]
            idx_in_zigzag = curI+(1:(h-abs(i)));
            if (i<0 && mod(i,2)==0) || (i>0 && mod(i,2)==1)  % reverse order
                idx_d1(fliplr(idx_in_zigzag)) = diag(matI,i)';
            else
                idx_d1(idx_in_zigzag) = diag(matI,i)';
            end
            curI=curI+h-abs(i);
        end
        P_d1 = I_NN(:,idx_d1);
        ...
        H1=P_d1*blkdiag(eye(h),kmat1(NN-h)')*P_alpha;
    case 'as'
        idx_d2 = zeros(1,NN);
        idx_d2(1:h)=diag(fliplr(matI),0)';
        curI=h;
        for i=[-h+1:-1, 1:h-1]
            idx_in_zigzag = curI+(1:(h-abs(i)));
            if (i<0 && mod(i,2)==0) || (i>0 && mod(i,2)==1) 
                idx_d2(fliplr(idx_in_zigzag)) = diag(fliplr(matI),i)';
            else
                idx_d2(idx_in_zigzag) = diag(fliplr(matI),i)';
            end
            curI=curI+h-abs(i);
        end
        P_d2 = I_NN(:,idx_d2);
